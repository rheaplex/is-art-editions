...
contract DemocraticPalette {
    ...
    function updatePalette (uint8 red, uint8 green, uint8 blue, uint _votes)
        internal {
        if (_votes > palette_lowest_vote) {
            ...
            if (! already_present) {
                for (; j < palette.length; j++) {
                    if (palette[j].votes == palette_lowest_vote) {
                        palette[j] = Colour(red, green, blue, _votes);
                        break;
                    }
                }
            }
            ...
            if (already_present) {
                emit PaletteChanged(i);
            } else {
                emit PaletteChanged(j);
            }
        }
    }
